    ...
    <div hlmCard class="p-6">
      ...
      <div class="space-y-2 mb-4">
        <div class="flex justify-between text-sm">
          <span class="text-muted-foreground" data-testid="cart-item-count"
            >{{ totalItems() }} item{{ totalItems() > 1 ? 's' : '' }}</span
          >
          <span class="text-foreground">{{
            totalPrice() | currency: 'CHF' : 'symbol' : '1.2-2'
          }}</span>
        </div>
      </div>
      ...
      <div class="flex justify-between items-center mb-6">
        ...
        <span class="text-xl font-bold text-card-foreground" data-testid="cart-total">{{
          totalPrice() | currency: 'CHF' : 'symbol' : '1.2-2'
        }}</span>
      </div>
